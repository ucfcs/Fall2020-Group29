...
from pymongo import (
    ReturnDocument,
)  # so that we can return the updated version of the document after updating it
...
@app.route(
    "/add_question", methods=["POST"]
)  # add a question to the database with Name, Responses, an Entities, returns this new document
def add_question(
    Name="BS-to-MS",
    Responses=["In order to dah da da da da"],
    Entities=["BS-to-MS", "How", "Sign Up"],
):
    new_question = {"Name": Name, "Responses": Responses, "Entities": Entities}
    InsertOneResult_Obj = mongo.db.questions.insert_one(
        new_question
    )  # insert_one() doesn't return a document, it returns a result that contains the ObjectID
    new_question.update(
        {"_id": str(InsertOneResult_Obj.inserted_id)}
...
@app.route(
    "/update_question", methods=["PUT"]
)  # give an existing question a file, returns updated document
def update_question(
    Entities=["BS-to-MS", "How", "Sign Up"],
    itemToUpdate="Name",
    newContents="why hello there",
):
...
@app.route(
    "/put_file", methods=["PUT"]
)  # give an existing question a file, returns updated document
def put_file(
    Entities=["BS-to-MS", "How", "Sign Up"],
    file="https://www.cs.ucf.edu/wp-content/uploads/2020/04/CSIT-Elective-List-AY2020-2021.pdf",
):
    updated = mongo.db.questions.find_one_and_update(
        {"Entities": {"$all": Entities}},
        {"$set": {"file": file}},
...
@app.route(
    "/put_contact", methods=["PUT"]
)  # give an existing question a contact, returns updated document
...
### contacts collection ###
@app.route(
    "/add_contact", methods=["POST"]
)  # add a contact to the database with Name, Title, Email, Phone and Office, returns inerted doctment
def add_contact(
    Name="Mark Heinrich",
    Title="CS Advisor",
    ...
    InsertOneResult_Obj = mongo.db.contacts.insert_one(
        new_contact
    )  # insert_one() doesn't return a document, it returns a result that contains the ObjectID
    new_contact.update(
        {"_id": str(InsertOneResult_Obj.inserted_id)}
    )  # append new_contact with the ObjectID (as a string) so that we can actually return something that resembles a document :/

    return jsonify(new_contact)


@app.route(
    "/update_contact", methods=["PUT"]
)  # give an existing contact a file, returns updated document
def update_contact(
    Name="Mark Heinrich",
    Title="CS Advisor",
    itemToUpdate="Name",
    newContents="Mark Heinrich 2",
):
    updated = mongo.db.contacts.find_one_and_update(
        {"$and": [{"Name": Name}, {"Title": Title}]},
        {"$set": {itemToUpdate: newContents}},
        upsert=False,  # upsert = if thing does not exist, make it exist
        return_document=ReturnDocument.AFTER,  # need this or else it returns the document from before the update
    )
...
if __name__ == "__main__":
    app.run(
        debug=True
    )  # run in debug mode so that when the .py updates, the whole thing relaunches too
